...
| ADR-004 | **Bright Data primary, Apify fallback + stacked free tier** — *pre-spike estimate, **superseded for TikTok by ADR-024** below; unaffected for Instagram (Apify was always primary there, see §3.1/§4)* | Lowest $/record, biggest free tier, pay-only-for-delivered; Apify offers documented recency sorting as the freshness escape hatch |
...
| ADR-024 | **Final provider routing from real Phase 1/1B measurements (supersedes ADR-004 for TikTok):** TikTok primary discovery = **Apify** (`clockworks/tiktok-scraper`, `searchQueries` + `searchSection:"/video"` + `videoSearchSorting:"LATEST"` + `videoSearchDateFilter:"PAST_24_HOURS"`); TikTok fallback = **Bright Data** (`discover_by=keyword`, dataset `gd_lu702nij2f790tmv9h`); TikTok refresh = **Apify** (`postURLs`). Instagram primary discovery = **Apify** (`apify/instagram-hashtag-scraper`); Instagram fallback/refresh = **none yet** (no verified Bright Data endpoint; the tested Apify actor has no URL-input field) — not a blocker, revisit when refresh-by-URL is actually needed. Bright Data Instagram is explicitly `UNSUPPORTED` in the normalizer dispatch, not silently unhandled. | Apify hashtag-mode TikTok discovery (the ADR-004-era assumption) measured 534.3h median age / 3.3% <24h — unusable for "taking off today." Apify **search mode** measured 2.5h median / 100% <24h. Bright Data TikTok keyword discovery measured 743.9h median / 16.7% <24h — real, working, but not fresh enough to be primary; kept as fallback for its field coverage and because the account being reachable at all was in question during Phase 1. See `docs/PROVIDER_SPIKE.md` for full measurements. |
